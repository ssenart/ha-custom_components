...
class NetatmoCamera(NetatmoBase, Camera):
    ...
    def handle_event(self, event):
        ...
        if data["home_id"] == self._home_id and data["camera_id"] == self._id:
            ...
            elif data[WEBHOOK_PUSH_TYPE] in ["NACamera-on"]:
                self.is_streaming = True
                self._status = "on"
            elif data[WEBHOOK_PUSH_TYPE] in ["NACamera-off"]:
                self.is_streaming = False
                self._status = "off"
            elif data[WEBHOOK_PUSH_TYPE] in ["NOC-on"]:
                self.is_streaming = True
                self._status = "on"
            elif data[WEBHOOK_PUSH_TYPE] in ["NOC-off"]:
                self.is_streaming = False
                self._status = "off"        
